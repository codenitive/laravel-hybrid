...
class Image {
	 ...
	 * @return  Image_Driver
	 */
	public static function make($config = array(), $filename = null)
	{
		...
		{
			throw new Exception('Driver '.$protocol.' is not a valid driver for image manipulation.');
		}
		
		$return = new $class($config);
		
		if ( ! is_null($filename)) $return->load($filename);
	...
	public static function config($index = array(), $value = null)
	{
		...
		{
			if ( ! is_null($value)) $index = array($index => $value);
			if (is_array($index)) static::$config = array_merge(static::$config, $index);
